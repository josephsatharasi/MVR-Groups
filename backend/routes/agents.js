...
router.post('/', async (req, res) => {
  try {
    // Validate cadre ID exists
    const cadre = await Cadre.findOne({ cadreId: req.body.cadreId });
    if (!cadre) {
      return res.status(400).json({ message: 'Invalid Cadre ID. Cadre not found.' });
    }
...
router.put('/:id', async (req, res) => {
  try {
    // Validate cadre ID exists if it's being updated
    if (req.body.cadreId) {
      const cadre = await Cadre.findOne({ cadreId: req.body.cadreId });
      if (!cadre) {
        return res.status(400).json({ message: 'Invalid Cadre ID. Cadre not found.' });
...
// Validate cadre ID
router.post('/validate-cadre', async (req, res) => {
  try {
    const { cadreId } = req.body;
    const cadre = await Cadre.findOne({ cadreId });
    if (cadre) {
      res.json({ valid: true, cadre: { name: cadre.name, role: cadre.cadreRole } });
    } else {
